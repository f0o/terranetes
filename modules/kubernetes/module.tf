...
ExecStart=/usr/lib64/coreos/kubelet-wrapper $KUBELET_LABELS_ARGS ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_KUBECONFIG_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_SYSTEM_PODS_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_NETWORK_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_DNS_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_AUTHZ_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_CGROUP_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_CADVISOR_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_CERTIFICATE_ARGS} ${local.configs[element([for v in local.config_mapping : v.key if v.short == local.k8s.nodes[count.index].version_short], 0)].kubelet.KUBELET_EXTRA_ARGS}
...
ExecStartPre=/bin/sh -c "echo KUBERNETES_VERSION=${local.k8s.nodes[count.index].version} | tee -a /etc/environment"
